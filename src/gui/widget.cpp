...
WidgetComposite::WidgetComposite(Widget* parent,
...
void WidgetComposite::GetFocus(Widget* widget)
{
...
bool WidgetComposite::AddChild(Widget* child)
{
    if(numChildren >= 100)
        return false;
...
void WidgetComposite::Draw(GraphicsContext* gc)
{
    Widget::Draw(gc);
...
void WidgetComposite::OnMouseDown(int32_t x, int32_t y, common::uint8_t button)
{
...
void WidgetComposite::OnMouseUp(int32_t x, int32_t y, common::uint8_t button)
{
    for(int i = 0; i < numChildren; ++i)
        if(children[i]->ContainsCoordinate(x - this->x, y - this->y))
        {
...
void WidgetComposite::OnMouseMove(int32_t oldx, int32_t oldy, int32_t newx, int32_t newy)
{
    int firstchild = -1;
    for(int i = 0; i < numChildren; ++i)
...
void WidgetComposite::OnKeyDown(char str)
{
    if(focussedChild != 0)
        focussedChild->OnKeyDown(str);
}

void WidgetComposite::OnKeyUp(char str)
{
    if(focussedChild != 0)
